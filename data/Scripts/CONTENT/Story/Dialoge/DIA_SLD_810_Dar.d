...
FUNC VOID DIA_Dar_PERM_Info()
{
	...
	&& (Dar_einmal == FALSE)
	{
		AI_Output (self, other, "DIA_Dar_PERM_03_01"); //Sometimes I let some vengeful swampweed junkies beat me up..
		Dar_einmal = TRUE;
	}
		else //DEFAULT
	{
...
FUNC VOID DIA_Dar_DuDieb_Info()
{
	...
	AI_Output (self, other, "DIA_Dar_DuDieb_03_01"); //(laughs)
	AI_Output (other, self, "DIA_Dar_DuDieb_15_02"); //Would you know anything about that?
	AI_Output (self, other, "DIA_Dar_DuDieb_03_03"); //No.
...
FUNC INT DIA_Dar_WoPaket_Condition()
{
	...
	&& ((Dar_Dieb == FALSE) || (Dar_Dieb == 1 && self.aivar[AIV_LastFightAgainstPlayer] == FIGHT_LOST))
	&& (MIS_Cipher_Paket == LOG_RUNNING)
	{
		return TRUE;
	};
...
{
	AI_Output (other, self, "DIA_Dar_WoPaket_15_00"); //Where's the package?
	...
	else
	{
		...
		Dar_Dieb = 1;
		B_LogEntry (Topic_CipherPaket,"Dar does not seem to be willing to reveal the whereabouts of the package. Perhaps he needs some convincing... ");
	};
};
...
func void DIA_Dar_ORCRING_necken ()
{
	...
	AI_Output			(self, other, "DIA_Dar_ORCRING_necken_03_01"); //Oh, come on, shut up. Who are you anyway?
...
func void DIA_Dar_ORCRING_no ()
{
	...
	AI_Output			(self, other, "DIA_Dar_ORCRING_no_03_01"); //Of course not. I'd have been amazed.
	Info_ClearChoices	(DIA_Dar_ORCRING);
};
...
func void DIA_Dar_BRINGORCELITERING_geld ()
{
	...
	if ((hero.guild == GIL_SLD) || (hero.guild == GIL_DJG))
	{
		AI_Output			(self, other, "DIA_Dar_BRINGORCELITERING_geld_03_01"); //Hmm. 600 gold coins?
