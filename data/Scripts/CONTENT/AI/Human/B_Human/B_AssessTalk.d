...
func void B_AssessTalk ()
{
	...
	if (self.guild < GIL_SEPERATOR_HUM)
	{
		...
		// PowerGamer: When the player stands on the other side of a guarded passage:
		// - if a gate guard is facing player (can see player): B_AssessPlayer -> Npc_CheckInfo -> DIA_XXX_Condition -> SetRefuseTalk -> B_AssessTalk
		// - if a gate guard is facing AWAY from player (can NOT see player): B_AssessTalk is called directly and SetRefuseTalk is NOT called!
		if (C_NpcIsGateGuard (self))
		{
			// Make sure SetRefuseTalk is called in DIA_XXX_Condition if need be
