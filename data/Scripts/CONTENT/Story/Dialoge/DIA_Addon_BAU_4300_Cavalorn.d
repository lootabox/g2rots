...
func void DIA_Addon_Cavalorn_HALLO_Info ()
{
	...
	AI_Output			(self, other, "DIA_Addon_Cavalorn_HALLO_08_01"); //Damn. I don't know where they're all hiding. You kill one, and shortly afterwards they're all back again.
	AI_Output			(self, other, "DIA_Addon_Cavalorn_HALLO_08_02"); //Wait a moment. I know you. You're the fellow who was constantly begging arrows from me in the Valley of Mines.
...
func void DIA_Addon_Cavalorn_HALLO_Bauern ()
{
	...
	AI_Output			(self, other, "DIA_Addon_Cavalorn_HALLO_Bauern_08_01"); //Really? Hmm. I wish I could follow you, but I have to finish my duties here first.
...
/*
PowerGamer: below is a description of the bug in the Daedalus interpreter that affected the next statement in this script file.
func int foo1()
{
	return TRUE;
};
func int foo2()
{
	foo1(); // BUG: return value of foo1 should be discarded but instead is pushed onto the stack of expression operands!
	return TRUE;
};
func void foo3()
{
...
During the evaluation of the expression foo2() == FALSE stack of operands will contain:
	   top: TRUE  - return value of foo2
	        TRUE  - return value of foo1 - BUG: this value is not supposed to be in the stack!
	bottom: FALSE - second operand of ==
Operator == compares last two operands on the stack: TRUE == TRUE - incorrect!

WORKAROUND 1: if only one of the operands of the binary operator is a function call - function call must be the first operand.
During the evaluation of the expression (foo2() == FALSE) stack of operands will contain:
	   top: FALSE - second operand of ==
	        TRUE  - return value of foo2
	bottom: TRUE  - return value of foo1 - BUG: this value is not supposed to be in the stack!
Operator == compares last two operands on the stack: FALSE == TRUE - correct.

WORKAROUND 2: do not use function calls as an operand of binary operator.
func void foo3()
{
	if foo2() // no binary operators used, last value on the stack is passed to if statement
...
{
	var int tmp;
	tmp = foo2(); // result of the assignment operator is correctly discarded and not pushed into stack
	if tmp == FALSE

WORDAROUND 3: make sure called function does not have expressions with unused value.
func int foo2()
{
	var int tmp;
	tmp = foo1(); // value of this expression is discarded
	return TRUE;
...
func int foo2()
{
	if foo1() {}; // value of this expression is used in the if statement
	return TRUE;
*/
	if (FALSE == B_GiveInvItems (other, self, ItSe_ADDON_CavalornsBeutel,1)) // WORKAROUND for Daedalus interpreter bug - DO NOT rearrange operands of equality operator!
		{
...
func void DIA_Addon_Cavalorn_ARTEFAKT_Info ()
{
	...
	AI_Output	(self, other, "DIA_Addon_Cavalorn_ARTEFAKT_08_01"); //Yes! They beat me down and tossed me to the goblins for lunch.
...
func void DIA_Addon_Cavalorn_HELFEN_Info ()
{
	...
	&& (hero.guild == GIL_NONE) 
	{
		AI_Output	(self, other, "DIA_Addon_Cavalorn_HELFEN_08_01"); //Maybe. But as scrawny as you look, you surely haven't held a proper sword in your hands for weeks.
	};
...
func void DIA_Addon_Cavalorn_AUSRUESTUNG_Info ()
{
	...
	AI_Output	(self, other, "DIA_Addon_Cavalorn_AUSRUESTUNG_08_05"); //I still have two healing potions here. Interested?
...
func void B_Addon_Cavalorn_VatrasBrief ()
{
	...
	if (MIS_Addon_Cavalorn_Letter2Vatras != LOG_SUCCESS)
	{
		...
		AI_Output	(self, other, "DIA_Addon_Cavalorn_VatrasBrief_08_05"); //Hmm, why not. You could take the letter into the city.
	};
